...
import { CheckCircle, Circle, Download, ChevronDown, ChevronUp, Lightbulb, ListTodo, Clock, Sparkles, CheckSquare } from 'lucide-react'
      ...
      <div className="max-w-5xl mx-auto">
        <header className="mb-8 flex justify-between items-end bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] font-bold uppercase tracking-wider rounded-md">v2.0</span>
        ...
        <div className="space-y-12">
          {Object.entries(groupedTasks).map(([sheetName, sheetTasks]) => {
            ...
            return (
              <section key={sheetName} className="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden">
                <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/20">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-xl ${progress === 100 ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}`}>
                      {sheetName.split(' ')[0]}
                    </div>
                    <div>
                      <h2 className="text-2xl font-black text-gray-800">{sheetName.split(' ').slice(1).join(' ')}</h2>
                  ...
                  <div className="flex items-center gap-8">
                    <div className="flex flex-col items-end">
                      <span className={`text-2xl font-black ${progress === 100 ? 'text-green-500' : 'text-blue-600'}`}>{progress}%</span>
                    </div>
                    <div className="relative w-40 h-3 bg-gray-100 rounded-full overflow-hidden border border-gray-50">
                      <div 
                        className={`h-full transition-all duration-1000 ease-in-out ${progress === 100 ? 'bg-green-500' : 'bg-blue-600 shadow-[0_0_12px_rgba(37,99,235,0.4)]'}`}
